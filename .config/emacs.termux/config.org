...
#+begin_src bash :tangle exwm/install-repeatable.sh :shebang #!/data/data/com.termux/files/usr/bin/bash
  ...
  pkg install vim emacs git neofetch man openssh
  pip install yt-dlp

  # install desktop utils
  pkg install x11-repo termux-api tigervnc
...
#+begin_src bash :tangle exwm/one-time-install-script.sh :shebang #!/data/data/com.termux/files/usr/bin/bash
  ...
  repeatable() {
      ...
      echo -en "\nRun install-repeatable.sh script? /n(updates and installs all packages) \n> "
      read in
      if [[ "$in" == "y" ]]; then
  ...
  symlinks() {
      ...
      read in
      if [[ "$in" == "y" ]]; then
	  ln -s ~/dotfiles2/.config/emacs.termux/exwm/.bashrc ~/.bashrc
	  ln -s ~/dotfiles2/.config/emacs.termux/exwm/termux-properties ~/.termux/termux-properties
	  ln -s ~/dotfiles2/.config/emacs.termux/exwm/vnc-config ~/.vnc/config
	  ln -s ~/dotfiles2/.config/emacs.termux/exwm/startdesktop ~/.local/bin/startdesktop
	  ln -s ~/dotfiles2/.config/emacs.termux/exwm/xstartup ~/.vnc/xstartup
      fi
