...
#+begin_src bash :tangle exwm/install-repeatable.sh :shebang #!/bin/bash

  pkg upgrade

  # install common stuff
  pkg install vim emacs git neofetch man openssh
  pip install yt-dlp x11-repo
...
#+begin_src bash :tangle exwm/one-time-installation-script.sh
